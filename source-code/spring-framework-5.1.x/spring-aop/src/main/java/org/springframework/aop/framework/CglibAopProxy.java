...
class CglibAopProxy implements org.springframework.aop.framework.AopProxy, Serializable {
	...
	public CglibAopProxy(org.springframework.aop.framework.AdvisedSupport config) throws org.springframework.aop.framework.AopConfigException {
		...
		if (config.getAdvisors().length == 0 && config.getTargetSource() == org.springframework.aop.framework.AdvisedSupport.EMPTY_TARGET_SOURCE) {
			throw new org.springframework.aop.framework.AopConfigException("No advisors and no TargetSource specified");
		}
		this.advised = config;
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			enhancer.setInterfaces(org.springframework.aop.framework.AopProxyUtils.completeProxiedInterfaces(this.advised));
			...
			// 设置多个回调函数，和一个回调过滤器
			// 回调函数是在目标方法被调用前被执行,回调过滤器用于选择使用哪一个回调函数 (这部分属于Cglib的API)
			...
			throw new org.springframework.aop.framework.AopConfigException("Could not generate CGLIB subclass of " + this.advised.getTargetClass() +
					": Common causes of this problem include using a final class or a non-visible class",
					ex);
		}
		catch (Throwable ex) {
			// TargetSource.getTarget() failed
			throw new org.springframework.aop.framework.AopConfigException("Unexpected AOP exception", ex);
		}
	}
	...
	public boolean equals(Object other) {
		return (this == other || (other instanceof CglibAopProxy &&
				org.springframework.aop.framework.AopProxyUtils.equalsInProxy(this.advised, ((CglibAopProxy) other).advised)));
	}
	...
	private static class AdvisedDispatcher implements Dispatcher, Serializable {
		...
		public AdvisedDispatcher(org.springframework.aop.framework.AdvisedSupport advised) {
	...
	private static class EqualsInterceptor implements MethodInterceptor, Serializable {
		...
		public EqualsInterceptor(org.springframework.aop.framework.AdvisedSupport advised) {
			this.advised = advised;
		}
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) {
			...
			if (other instanceof Factory) {
				...
				org.springframework.aop.framework.AdvisedSupport otherAdvised = ((EqualsInterceptor) callback).advised;
				return org.springframework.aop.framework.AopProxyUtils.equalsInProxy(this.advised, otherAdvised);
			}
			else {
				return false;
			}
	...
	private static class HashCodeInterceptor implements MethodInterceptor, Serializable {
		...
		public HashCodeInterceptor(org.springframework.aop.framework.AdvisedSupport advised) {
			this.advised = advised;
		}
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		public DynamicAdvisedInterceptor(org.springframework.aop.framework.AdvisedSupport advised) {
			this.advised = advised;
		}
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				// 1.exposeProxy属性处理
				// 如果exposeProxy配置为true，那么会将代理对象放入到threadLocal中，
				// 用于解决AOP内部方法调用时失效的问题,(AopContext.currentProxy获取代理对象,然后用这个代理对象调用被代理的方法即可解决)
				...
				// 2.获取通知
				// 获取可以作用到要调用的方法上的通知
				// 在创建代理对象的时候 获取通知的维度是类级别，而此处获取的维度是方法级别
				...
				if (chain.isEmpty() && Modifier.isPublic(method.getModifiers())) {
					...
					Object[] argsToUse = org.springframework.aop.framework.AopProxyUtils.adaptArgumentsIfNecessary(method, args);
	 ...
	 */
	private static class CglibMethodInvocation extends org.springframework.aop.framework.ReflectiveMethodInvocation {
	...
	private static class ProxyCallbackFilter implements CallbackFilter {
		...
		public ProxyCallbackFilter(
				org.springframework.aop.framework.AdvisedSupport advised, Map<String, Integer> fixedInterceptorMap, int fixedInterceptorOffset) {
		...
		public boolean equals(Object other) {
			...
			org.springframework.aop.framework.AdvisedSupport otherAdvised = otherCallbackFilter.advised;
			...
			if (!org.springframework.aop.framework.AopProxyUtils.equalsProxiedInterfaces(this.advised, otherAdvised)) {
				return false;
			}
