...
public class ClassPathMapperScanner extends ClassPathBeanDefinitionScanner {
  ...
  public Set<BeanDefinitionHolder> doScan(String... basePackages) {
    ...
    if (beanDefinitions.isEmpty()) {
      ...
      // 修改Bean定义，主要是将BeanClass修改为MapperFactoryBean.class
      // 由这个工厂Bean统一实例化所有这些对象
      // 例如有一个CustomDAO的接口，其Bean定义的BeanClass为CustomDAO
      // 那么此处会将其替换为MapperFactoryBean，因为Spring无法为接口实例化，所以此处会将其替换为一个工厂Bean
      // 等到第一次执行这个CustomDAO接口的方法时，再去创建代理对象
